...
router.post('/getCartList',function(req, res)
{
		...
		.then(function(result){
			if(result.length == 1)
			{
				...
				query = "Select Welcome.ProductID,ProductName, Brand, Price, ProductsInCart.Amount from Welcome join ProductsInCart on Welcome.ProductID = ProductsInCart.ProductID where ProductsInCart.CartID ='" + cart + "'";
				db.Select(query)
				.then(function(result)
				{
					res.send(result);
...
let getPrice = function (pID) {
	...
	query= " Select price FROM Welcome WHERE ProductID='" +pID+ "'";
...
let getTotalCartPrice = function (cID) {
	...
	query= " Select SUM(price*Amount) AS total FROM ProductsInCart AS sc JOIN Welcome as pr ON sc.ProductID=pr.ProductID WHERE CartID='" +cID+ "'";
	return db.Select (query)
};
